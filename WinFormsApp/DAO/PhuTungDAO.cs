...
namespace WinFormsApp.DAO
{
    internal class PhuTungDAO
    {
        ...
        public void DeletePhuTungByName(string tenVTPT)
        {
            string query = "DELETE FROM dbo.PHUTUNG WHERE TenVTPT = @TenVTPT";
            ...
            using (SqlConnection connection = DataProvider.instance.getConnect())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand(query, connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@TenVTPT", tenVTPT);

                        int rowsAffected = cmd.ExecuteNonQuery();

                        if (rowsAffected == 0)
                        {
                            throw new InvalidOperationException($"Không tìm thấy phụ tùng với tên: {tenVTPT}");
                        }
                    }
                }
                catch (SqlException sqlEx)
